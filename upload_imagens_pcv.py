"""
Realiza o upload de imagens para produtos da loja que contenham variações de modelos.
PVC = Produtos Com Variações
...
respectivos modelos.
As pastas devem ser nomeadas seguindo o seguinte padrão: [código do produto]-[nome do modelo]
"""
...
def upload_images(code: str, model: str, image_list: list[str]) -> bool:
    """
    Realiza o post/upload das imagens convertidas para a loja na plataforma ORUC

        :param code: O código do produto na plataforma
        :param model: O modelo do produto ao qual as imagens pertencem
...
root_dir = r"../../../Downloads/produto-img-modelo"
...
for root, directories, files in walk(root_dir):
    for dir_name in directories:
        ...
        prod_code, prod_model = dir_name.split('-')
        prod_image_list = []
        ...
        if upload_images(prod_code, prod_model, prod_image_list):
